...
# 삭제되는 맵에 연결된 스폰 지점과 코어도 함께 제거합니다.
...
execute if entity @e[type=minecraft:marker,tag=td.map.remove,limit=1] as @e[type=minecraft:marker,tag=td.core.point] if score @s td.map_id = @e[type=minecraft:marker,tag=td.map.remove,limit=1] td.map_id run tag @s add td.core.remove
